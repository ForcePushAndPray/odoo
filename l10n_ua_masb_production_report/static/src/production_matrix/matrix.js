...
export class ProductionMatrix extends Component {
    static props = standardWidgetProps;
    static template = "l10n_ua_masb_production_report.ProductionMatrix";
    ...
    rowClass(row) {
        ...
        if (row.kind === "total") {
            classes.push("o_masb_prod_total");
        } else if (row.kind === "group") {
            classes.push("o_masb_prod_group");
        }
        return classes.join(" ");
    }
    ...
    cellClass(row, column, index, value) {
        ...
        if (row.kind === "element" && index === 1) {
            classes.push("o_masb_prod_indent");
        }
        if (this.isDrillable(row, column, value)) {
            classes.push("o_masb_prod_drillable");
        }
        return classes.join(" ");
    }
}

export const productionMatrix = { component: ProductionMatrix };

registry.category("view_widgets").add("masb_production_matrix", productionMatrix);
